...
#+STARTUP: beamer
#+AUTHOR: Marcos Benevides \href{https://github.com/schonfinkel}{\faGithub} \href{https://www.linkedin.com/in/schonfinkel/}{\faLinkedin}
#+DATE: \today
#+DESCRIPTION: 
#+KEYWORDS: 
#+LANGUAGE:  pt
...
#+ATTR_BEAMER: :overlay +-
+ [[https://github.com/Dr-Nekoma][Dr. Nekoma]]: Projeto dedicado à programação recreativa e exploração de tópicos
  nichados e sem influências mainstream. Comumente se materializa na forma de
  projetos bizarros (praticidade questionável).

+ [[https://github.com/dont-rely-on-nulls][Dont Rely on Nulls]]: Uma iniciativa que se destina a empreendimentos mais
  sérios. Por via de regra, projetos do "Dr. Nekoma" que se tornem sérios acabam
  sendo migrados e aprimorados nessa organização (i.e. [[https://github.com/dont-rely-on-nulls/zerl][Zerl]]).
  
** Contexto

#+ATTR_BEAMER: :overlay +-
+ Este é um ambiente recreativo.
+ Temos toda a liberdade que não nos é permitida durante (a maioria) dos
  empregos de tempo integral.
+ Quais são as coisas que sonhamos em ter?
+ Como podemos fazer isso acontecer?

** Missão

O projeto deve ter as seguintes características:

#+ATTR_BEAMER: :overlay +-
1. Deve ser rápido (considerando que é um jogo), mas sem precisar recorrer ao Rust.
2. Gráficos fáceis sem a dor de cabeça das APIs gráficas.
3. Uma fonte única de verdade.
4. Server distribuído e tolerante a falhas.
5. Ambiente de desenvolvimento reproduzível, isolado e declarativo, que assume
   apenas a existência de algum editor de texto e 0 pacotes instalados
...
|-----------------------------------------+-----------------------|
| Deve ser rápido                         | [[https://ziglang.org/][Zig]]                   |
| Gráficos fáceis                         | [[https://www.raylib.com/][Raylib]]                |
| Uma fonte única de verdade              | [[https://www.postgresql.org/][PostgreSQL]] + [[https://omnigres.com/][Omnigres]] |
| Server distribuído e tolerante a falhas | [[https://www.erlang.org/][Erlang/OTP]]            |
...
#+ATTR_BEAMER: :overlay +-
+ [[https://ziglang.org/][Zig]]: Uma linguagem de programação focada em *desempenho*, *segurança* e *controle
  explícito* sobre a memória, também fornece boa *interoperabilidade* com C.

+ [[https://www.raylib.com/][raylib]]: Uma biblioteca para criar aplicativos gráficos e jogos, que fornece
  uma API simples para gráficos, entrada e áudio.

    #+CAPTION: Lyceum, pré v0.1.0.
...
#+ATTR_BEAMER: :overlay +-
+ ~C Nodes~ são programas que atuam como nós ocultos dentro de um sistema Erlang
  distribuído, permitindo que aplicações escritas em linguagens não suportadas
  nativamente pela BEAM participem de clusters Erlang.
+ Utilizam a biblioteca ~Erl_Interface~ para se comunicar com outros nós via
  TCP/IP, como se fossem nós Erlang nativos.
...
+ Inicialmente uma biblioteca interna do jogo, foi criada para traduzir de forma
  idiomática tipos em Zig \rightarrow Erlang (e vice versa).
+ Depois de várias iterações e redesigns acabou virando um pacote separado.
...
+ Uma ~Erlang Node~ é uma instância em execução do Erlang Runtime System (ERTS).
+ Processos em diferentes nós podem enviar e receber mensagens entre si.
...
#+CAPTION: Adatapção de [cite: @cesarini2009erlang p.107 chap.4]
...
#+CAPTION: A função ~spawn/3~ cria um processo do módulo "~Mod~", rodando a função "~FunName~" com argumentos ~[Arg1, ..., ArgN]~, e retorna o Id do novo Processo (~Pid2~).
#+begin_src erlang
...
#+ATTR_LATEX: :width 0.6\linewidth
#+CAPTION: Adatapção de [cite:@cesarini2016designing p.29 chap.2]
...
#+BEGIN_SRC dot :file ./img/erlang_message_passing.png :cmdline -Kdot -Tpng :exports results
  digraph Send {
    ...
    pid1 [label="Pid 1", xlabel="Pid2 ! { hello, self() }", fontsize="10pt", style=filled, fillcolor=grey];
    pid2 [label="Pid 2", fontsize="10pt", style=filled, fillcolor=grey, xlabel="receive (...) end." ];
...
#+BEGIN_QUOTE
OTP é um conjunto de frameworks, princípios e padrões que guiam e alicerçam a
estrutura, design, implementação e deploys de sistemas baseados em Erlang. Usar
OTP sem seus projetos vai te ajudar a evitar *complexidade acidental: coisas que
são difíceis por que você escolheu ferramentas inadequadas*.
...
+ Uma ~Application~ é um constructor básico para deploy de projetos em Erlang/OTP.
+ Pode ser tanto uma biblioteca que é utilizada por outros processos
  (i.e. ~Library Application~) ou um processo dotado da capacidade de ser
  iniciado/interrompido (implementando uma interface com ~start~ / ~stop~).

** OTP Supervisors

#+begin_quote
Uma árvore de supervisão é uma árvore de processos. Os processos superiores 
(~supervisors~) na árvore monitoram os processos menores (~workers~) na hierarquia e
os reiniciam em caso de falhas.

[cite:@armstrong2013 p.398 chapter.23 part.23.5]
#+end_quote

Independentemente do que os processos monitados façam, um supervisor funciona
sempre do mesmo modo e seu funcionao pode ser dividido em duas categorias:

+ Uma parte genérica que lida com a criação, monitoramento e ~restart~ de
  processos filhos.
+ Uma especificação de como os processos filhos devem ser iniciados e reiniciados.

** Estratégias de Supervisão (~one_for_one~)
...
** Estratégias de Supervisão (~one_for_all~)
...
+ Fornece uma interface para implementar processos que respondem a requisições
  síncronas (~call~) e assíncronas (~cast~).
...
+ Gerenciamento de ciclo de vida padronizado: inicialização e outras transições
  de estado são feitas de forma consistente.
+ Podem ser facilmente integrados em árvores de supervisão.
...
+ Oferece uma interface para implementar máquinas de estados finitos com estados
  e transições bem definidos.

#+CAPTION: Cada jogador no Lyceum é um processo com um ~gen_statem~.
#+ATTR_LATEX: :width 0.5\linewidth
...
#+ATTR_LATEX: :width 0.5\linewidth
#+CAPTION: Arquitetura estática com 1 ~application~ que inicia um ~supervisor~, que monitora *exatamente* 4 processos.
...
#+ATTR_LATEX: :width 0.5\linewidth
#+CAPTION: Arquitetura estática com 1 ~application~ que inicia 1 ~top level supervisor~, que monitora *exatamente* 1 ~supervisor~ interno e 3 outros *worker processes*. O ~supervisor~ interno monitora exatamente 2 outros processos.
...
#+ATTR_LATEX: :width 0.5\linewidth
#+CAPTION: Arquitetura dinâmica com 1 ~application~ que inicia 1 ~supervisor~ dinâmico, monitorando 0 processos.
...
#+ATTR_LATEX: :width 0.5\linewidth
#+CAPTION: Arquitetura dinâmica com 1 ~application~ e 1 ~supervisor~ dinâmico, monitorando 3 processos.
...
+ Módulos: O nível mais baixo de encapsulamento.
+ ~Applications~: Um agrupamento módulos, estrutura de nível intermediário.
+ ~Releases~: Um agrupamento de uma ou mais aplicações.
...
#+CAPTION: Uma única "release", múltiplas aplicações.
#+ATTR_LATEX: :width \linewidth
...
digraph Lyceum {
  ...
  subgraph cluster_client_1 {
    ...
    Z1 [label="Zerl", fillcolor="lightcoral"];
  ...
  subgraph cluster_client_2 {
    ...
    Z2 [label="Zerl", fillcolor="lightcoral"];
  ...
  subgraph cluster_client_3 {
    ...
    Z3 [label="Zerl", fillcolor="lightcoral"];
  ...
  subgraph cluster_client_n {
    ...
    subgraph player_2 {  
      ZN [label="Zerl", fillcolor="lightcoral"];
...
#+CAPTION: A ~character.view~ acaba operando como um módulo, sem expor detalhes de implemetação das tabelas originais.
...
+ Uma extensão para o PostgreSQL visa oferecer melhor qualidade de vida para o
  desenvolvimento de aplicações, oferece baterias para lidar com tráfego
  HTTP/WebSockets, roteamento e consultas reativas, etc.
+ Integrações avançadas: Inclui componentes como ~omni_httpd~ (servidor HTTP),
  ~omni_session~ (sessões), ~omni_vfs~ (sistema de arquivos virtual), ~omni_types~
...
#+CAPTION: Todas as dependências necessárias para rodar o interpretador do Python 3.13 
#+ATTR_LATEX: :width \linewidth
[[./img/nix_python.png]]
...
Nix é uma linguagem de programação pura, funcional, /lazy/, declarativa e reproduzível.

#+begin_quote
Reprodutibilidade é um conceito de design de software, onde uma operação para as
mesmas entradas produz a mesma saída.
#+end_quote

** Nix (Gerenciador de pacotes)

#+ATTR_BEAMER: :overlay +-
+ ~nix~ é um gerenciador de pacotes puramente funcional.
+ Trata pacotes como *valores* em linguagens de programação puramente funcionais.
+ Sempre compila todo o /fecho transitivo/ (ou árvore de dependências) de um
  pacote, porém, se uma dependência já estiver presente na ~/nix/store/~ (ou num
  cache remoto), o ~nix~ pode usá-la, o que significa que pode fornecer
...
#+ATTR_BEAMER: :overlay +-
+ Armazenamento imutável: Todos os pacotes, configurações e dependências residem
...
+ Cada caminho na ~/nix/store/~ inclui um hash computado a partir de toda a
  derivação do pacote (todas as entradas: código-fonte, dependências,
  configurações de compilação, ambiente, etc.), garantindo que até mesmo a menor
  alteração produza uma nova entrada de armazenamento isolada.
...
#+ATTR_BEAMER: :overlay +-
+ Garbage collection: Pacotes antigos ou não utilizados permanecem até serem
...
+ Sustenta o modelo funcional de gerenciamento de pacotes do Nix, permitindo
  *compilações reproduzíveis*, *atualizações atômicas* e *rollbacks*.

    #+ATTR_LATEX: :width 0.4\linewidth
    [[./img/nix_store.png]]

** Gerações (e rollbacks)
...
+ Distribuição GNU/Linux, completamente configurável e gerenciável via Nix
  (linguagem) e ~nix~ (gerenciador de pacotes).
+ /Infrastructure as code/ desde 2003.
+ O estado do sistema é uma saída de tudo que está sua configuração (em Nix).

** Nix não é Docker
#+BEGIN_QUOTE
"Mas nós já temos o Docker..."
#+END_QUOTE

#+ATTR_BEAMER: :overlay +-
+ Num ~Dockerfile~, você tem garantias de que um *mesmo comando* vai ser rodado na
  *mesma ordem*, ou seja:
  * Isso não significa que você vai ter o mesmo resultado *sempre*.
  * *Repetibilidade* não é *reprodutibilidade*.
...
Para desenvolvimento local, usamos uma combinação de Nix Shells e [[https://devenv.s/h][devenv.sh]]. A
escolha do ~devenv~ advém do fato dele melhorar a "developer experience" ao usar
Nix e é um ótimo jeito de usuários não avançados experimentarem todas as coisas
boas que Nix shells podem trazer.

#+ATTR_BEAMER: :overlay +-
+ Módulos para serviços populares como PostgreSQL, Redis, MySQL, RabbitMQ, etc.
...
ou, se você estiver usando [[https://github.com/direnv/direnv][direnv]] (com [[https://github.com/nix-community/nix-direnv][nix-direnv]])
...
#+ATTR_BEAMER: :overlay +-
+ O deploy do jogo é feito num projeto separado, o [[https://github.com/Dr-Nekoma/trashcan][Trashcan]].
+ O provisionamento da infra inicial é feito com [[https://opentofu.org/][Open Tofu]] e é divido em duas etapas:
  * ~bootstrap~: Onde colocamos uma versão com o mínimo de pacotes possível,
    apenas usuários iniciais a acesso via ssh.
  * ~nekoma~: Configuração com todos os *módulos* habilitados.
+ O processo pode ser replicado em 3 ambientes diferentes:
  * AWS e Magalu Cloud. Usando os módulos para terraform do [[https://github.com/nix-community/nixos-anywhere/tree/main/terraform][nixos-anywhere]].
  * Localmente, criando uma VM QEMU automaticamente com Nix.

** Nix Modules (I)

#+CAPTION: Exemplo de uma VM onde certos módulos são habilitados
...
O projeto [[https://github.com/nix-community/disko][disko]] possibilita o particionamento e formatação de discos de forma
*declarativa*.
...
#+ATTR_BEAMER: :overlay +-
+ O [[https://github.com/nix-community/impermanence][impermanence]] é outro módulo interessate da [[https://github.com/nix-community][nix-community]], que permite que
  apenas as partições e diretórios descritas na configuração em Nix sejam
  mantidas a cada reboot.
+ A inspiração é tentar tratar VMs como processos em Erlang.
...
digraph Lyceum {
  ...
  subgraph cluster_lyceum {
    ...
    subgraph cluster_devshell {
      ...
      DB [label="Nix Build", fillcolor="lightcoral"];
    }
  }

  subgraph cluster_infra {
    label="Trashcan (Infra)";
    style=filled;
    fillcolor="lightgoldenrodyellow";
    color="gold";
    penwidth=3;
...
#+ATTR_BEAMER: :overlay +-
+ Composição é uma propriedade fundamental para alavancar até mesmo as ideias
  mais bizzaras.
+ Fundamentação teórica sólida tem um efeito multiplicador em termos de
  produtividade.
  * Programação declarativa é naturalmente composicional.
+ Nix >>>>>>>> docker et al.
...
:BEAMER_ENV: block
:END:
- [[https://github.com/Dr-Nekoma/lyceum][Lyceum]]: O jogo criado para testar todo tipo de ideia maluca.
- [[https://github.com/dont-rely-on-nulls/zerl][Zerl]]: Uma biblioteca que surgiu dentro do jogo e depois virou algo maior.
- [[https://github.com/Dr-Nekoma/trashcan][Trashcan]]: O servidor declarativo.

*** Slides
:PROPERTIES:
:BEAMER_ENV: block
:END:
- [[https://github.com/dont-rely-on-nulls/gambiconf-2025][Gambiconf 2025]]: Uma "demo secreta", visto que todos os slides também foram
  feitos num ambiente Nix com Emacs + Orgmode + LaTeX, contém inclusive uma
  versão customizada do Emacs para buildar os slides em CI.
...
+ Reasoning about declarative programs is simple. (...) . Since declarative
  programs compute only values, simple algebraic and logical reasoning
  techniques can be used.

[cite: @van2004concepts p.111 chapter.3]
#+end_quote
