...
#+CAPTION: Representação esquemática de duas máquinas distintas, cada uma com nodes Erlang executando múltiplos aplicativos sobre a stack completa [cite: @areias2025 p.3 part.2]
...
+ ~Applications~: Um agrupamento de módulos, nível intermediário.
...
#+CAPTION: Garantindo que um equipamento está sendo usado da forma correta.
#+NAME: fig:sql-checks
#+ATTR_LATEX: :width 0.8\linewidth
[[./img/sql_checks.png]]
...
#+CAPTION: Configuração declarativa do PostgreSQL (com extensões).
#+ATTR_LATEX: :width 0.5\linewidth
...
#+ATTR_BEAMER: :overlay +-
+ O deploy do jogo é feito num projeto separado, o [[https://github.com/dont-rely-on-nulls/chessboard][Chessboard]].
...
#+CAPTION: Inicialização declarativa do PostgreSQL (usando ~systemd~).
#+ATTR_LATEX: :width 0.8\linewidth
