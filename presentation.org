...
*** Programação Declarativa :BMCOL:B_block:
    :PROPERTIES:
    :BEAMER_col: 0.5
    :END:

+ Programação Declarativa e Composição: Componentes que podem ser desenvolvidos
  e verificados de forma independente.
+ Boa fundamentação teórica \rightarrow Simplicidade.

Adaptação de [cite: @van2004concepts p.111 chapter.3].
...
+ Utilizam a biblioteca ~Erl_Interface~ (~ei.h~) para se comunicar com outros nós via
...
#+CAPTION: OTP fornece padrões e ferramentas que simplificam sua vida e evitam complexidade desnecessária causada por escolher ferramentas ruins. [cite:@cesarini2016designing p.4-5 chap.1].
#+ATTR_LATEX: :width 0.6\linewidth
[[./img/erlang_otp.png]]
...
#+BEGIN_SRC dot :file ./img/erlang-sup-callbacks.png :cmdline -Kdot -Tpng :exports results
digraph gen_server {
  rankdir=LR;
