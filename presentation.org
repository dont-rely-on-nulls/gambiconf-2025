...
Várias ideias podem ser encontradas em [cite: @van2004concepts p.111 chapter.3].
...
+ Quando você implementa as funções ~start/2~ e ~stop/1~ (no behaviour ~application~),
  o OTP agora é capaz de gerenciar o ciclo de vida de sua aplicação de *forma padronizada*.
...
#+ATTR_LATEX: :width 0.8\linewidth
#+CAPTION: Adaptação de [cite:@cesarini2016designing p.175 chap.8]
...
#+ATTR_LATEX: :width 0.7\linewidth
#+CAPTION: Adaptação de [cite:@cesarini2016designing p.58 chapter.3]
...
** Compondo múltiplos ~gen_statem~

#+CAPTION: Fonte - [[https://www.youtube.com/watch?v=endy-GrXov4][Automata Unveiled: A Theoretical Exploration of State Machines - Nelson Vides]] 
#+ATTR_LATEX: :width \linewidth
[[file:./img/erlang_gen_statem_composition.png]]
...
** Schemas + Views como Módulos

+ Os ~schemas~ do PostgreSQL são usados ​​como ~namespaces~.
+ Cada ~schema~ contém tabelas internas que não são acessadas diretamente.
...
+ Omnigres é uma extensão do PostgreSQL que adiciona suporte integrado a
  HTTP/WebSockets, roteamento e consultas reativas.
+ Inclui módulos avançados como ~omni_httpd~, ~omni_session~, ~omni_vfs~, ~omni_types~,
...
+ Cada caminho na ~/nix/store/~ inclui um hash computado a partir de toda a
  derivação do pacote (código-fonte, dependências, configurações de compilação,
  ambiente).
...
#+ATTR_BEAMER: :overlay +-
+ Para desenvolvimento local, usamos uma combinação de Nix Shells e [[https://devenv.s/h][devenv.sh]]
...
+ O provisionamento da infra inicial é feito com [[https://opentofu.org/][Open Tofu]] e [[https://github.com/nix-community/nixos-anywhere/tree/main/terraform][nixos-anywhere]].
