...
+ Uma ~Erlang Node~ é uma instância BEAM que pode executar diversos processos leves.
...
#+ATTR_LATEX: :width 0.6\linewidth
#+CAPTION: O processo ~Pid 2~ foi criado com uma call ~spawn/3~ do processo ~Pid 1~, adatapção de [cite:@cesarini2016designing p.29 chap.2]
...
#+ATTR_LATEX: :width 0.6\linewidth
#+CAPTION: O processo ~Pid 2~ recebendo a mensagem ~{ Pid1, hello }~ do processo ~Pid 1~.
...
+ São processos que ser iniciados, interrompidos e gerenciadas de forma independente.
